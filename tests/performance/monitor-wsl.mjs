 ...
 *
 * 用法:
 *   node tests/performance/monitor-wsl.mjs [秒数] [间隔秒]
 *   node tests/performance/monitor-wsl.mjs --until-file /tmp/stress.done [间隔秒]
 *
 * 与压测一起:
 *   DONE=/tmp/stress.done
 *   rm -f "$DONE"
 *   node tests/performance/monitor-wsl.mjs --until-file "$DONE" 1 &
 *   node tests/performance/stress-timesheet-submit.mjs 100 300
 *   touch "$DONE"; wait
 *
