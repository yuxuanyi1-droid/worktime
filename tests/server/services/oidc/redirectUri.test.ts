...
import { buildFixedRedirectUri, buildTrustedRedirectUri } from '@server/services/oidc/redirectUri';
...
describe('buildTrustedRedirectUri', () => {
  ...
  it('生产模式可关闭请求 Host 自动信任，只接受显式来源', () => {
    expect(() => buildTrustedRedirectUri(
      'https://forged-host.example/worktime',
  ...
  it.each([
    ...
    ['https://work.example.com/worktime?next=evil', '带 query 地址'],
    ['https://work.example.com/worktime#fragment', '带 hash 地址'],
    ['not-a-url', '无效地址'],
  ])('拒绝%s（%s）', (base) => {
  ...
  it('拒绝允许来源中的路径和危险协议配置', () => {
    expect(() => buildTrustedRedirectUri(
      'https://work.example.com',
      '',
      ['https://work.example.com/path'],
      false,
    )).toThrow('只能配置协议、域名和端口');
    expect(() => buildTrustedRedirectUri(
      'https://work.example.com',
      '',
      ['javascript:alert(1)'],
